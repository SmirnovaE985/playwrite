...
test('#6735 Создание обращения с причиной "Редактирование заказа"', async ({ page }) => {
...
test('#3242 Создание обращения с причиной "Справка"', async ({ page }) => {
  const page1 = await createAppeal(page);
await page1.locator('[data-test="select-appeal"]').click();
  await page1
  .locator('[data-test="select-appeal"] li')
  ...
  .click();
  await expect(page1.getByText('ИМКЦ Ресурс')).toBeVisible();
});
...
test('#3245 Создание обращения с причиной "Информация по заказу"', async ({ page }) => {
  const page1 = await createAppeal(page);
await page1.locator('[data-test="select-appeal"]').click();
  await page1
  .locator('[data-test="select-appeal"] li')
...
test('#4344 Переход из одной причины обращения в другую', async ({ page }) =>{
const page1 = await createAppeal(page);
await page1.locator('[data-test="select-appeal"]').click();
await page1
  .locator('[data-test="select-appeal"] li')
...
test('#6750 Редактирование ранее созданного заказа, через историю', async ({ page }) =>{
const page1 = await createAppeal(page);
await page1.locator('[data-test="go-appeal-history"]').click();
await page1
...
test('#5636 Создание обращение клиента, который не зарегистрирован в ПЛ, но обращался на линию', async ({ page }) => {
...
test("#4576 Создание обращения - валидация (телефон)", async ({ page }) => {
...
// https://allure.itlabs.io/project/28/test-cases/4576?treeId=58
test("#4576 Создание обращения - валидация (email)", async ({ page }) => {
  await page.goto("http://localhost:3000/home");
  await page.locator('input[name="login"]').fill("mmalyutina");
  await page.locator('input[name="password"]').fill("123456789");
  await page.getByRole("button", { name: "Войти" }).click();
  ...
  await submitBtn.click();
  await expect(page.getByText("Укажите корректный email")).toBeVisible();
  // 4) Валидный email -> переход на selectClient
...
// https://allure.itlabs.io/project/28/test-cases/4576?treeId=58
test("#4576 Создание обращения - валидация (мессенджер)", async ({ page }) => {
  await page.goto("http://localhost:3000/home");
  await page.locator('input[name="login"]').fill("mmalyutina");
  await page.locator('input[name="password"]').fill("123456789");
  await page.getByRole("button", { name: "Войти" }).click();
